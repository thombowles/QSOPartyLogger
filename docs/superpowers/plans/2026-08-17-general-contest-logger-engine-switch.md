...
- Constitution Article 4: this plan adds no party and changes no score, no export byte and no saved figure for any bundled party. Article 9: one contest (here: one task) per commit — a task that changes both a model type and its lowering is one structural refactor. Article 6: the README test count and the docs ship in the last task. Article 8: verification is by test, build and log.
...
```bash
xcodegen generate && set -o pipefail; TEST_RUNNER_QPL_RECORD_GOLDEN=1 xcodebuild test -project QSOPartyLogger.xcodeproj -scheme QSOPartyLogger -destination 'platform=macOS' -only-testing:QSOPartyLoggerTests/EngineEquivalenceTests/testRecordGoldenWhenAsked 2>&1 | grep -E "Test Case|Executed|error:|QPL_RECORD_GOLDEN_PATH" | tail -5
```
...
4. In the "Files and export" section (around line 830), one sentence on the log format: "Logs are saved in the schema-2 shape (each row carries its sent and received exchange as element maps; the document carries the entrant's side and fixed exchange). Every earlier `.qplog` opens unchanged; a log saved by this build needs this build or later, and a log from a newer build is refused rather than rewritten."
