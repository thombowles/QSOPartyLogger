...
| `dxccEntity` | `from: callsign \| receivedToken \| receivedTokenOrCallsign`, `list: arrl \| arrlPlusWAE`, `exclude: [codes]`, `countEntities: Bool`, `callsignOverrides: [setRef]?` (the enumerated sets whose token the worked callsign takes over when it resolves to that prefix's entity — `PA` from `PA0AAA` is the Netherlands, from `W3XYZ` Pennsylvania; applied before any resolver runs, so the token is never credited twice) | entity (label = primary prefix, identity = code / WAE name); when `countEntities` is false the literal `DX` |
...
**`CabrilloSpec`** — `{ contest, location: state | section | entrantToken, transmitterColumn: Bool, serialSequence: contest | perBand, categoryMode: String?, homeLocation: String?, reportColumn: Bool }`. `reportColumn` writes the generic template's ex1 slot — the row's report, or the mode's default — where the exchange declares no `rst`, `serial` or `name`: MDC, IDQP, NCQP and WIQP, whose logs have always carried one. `LOCATION:` — `state`: the entrant's state/province token (a party's sent `location`, resolved to its state for an inside entrant, exactly `MyLocation.entrantToken` today; otherwise `exchangeDefaults.state`, asked in Setup); `section`: the sent `section` element or `exchangeDefaults.section`; `entrantToken`: the sent `location` element verbatim. `homeLocation` is the token an entrant on the first-listed side writes where a contest names one for everybody inside — a party's primary state, so the 7th Call Area's eight member states all header its primary `AZ` rather than the entrant's own county's state (today's `MyLocation.entrantToken`, and why `PartyDefinition.validate()` insists the primary state is one of the member states); nil means resolve the entrant's own sent location to its group/state. An entrant whose callsign resolves outside USA/Canada writes `DX`. The QSO line is derived: `QSO: freq mo date time mycall <sent elements> call <received elements> [t]`, each element padded to its `cabrilloWidth`; sponsors state that spaces delimit and columns need not align, so widths matter only for our own byte-identity.
...
8. `ScoreBreakdown` gains `outOfTimeRowIDs`, `outOfTimeCount`, `operatedMinutes`, `offMinutes`; `MultKey` carries `classID: String` (with `multClass` kept as an optional view and the `multClass:` init as a bridge, so persisted snapshots and sidebar keys read unchanged); `workedValues(classID:)` and `countsByClassID` are the general readers, `classCounts` stays `[MultClass: Int]` for the party-shaped sidebar.
...
**`ContestLog`** — `partyID` (the Swift name and the JSON key both keep it), `sideID: String`, `sentExchange: [String: [String]]` (a list for county lines; every other element one value), `station`, `qsos`, `messages`, `operatingMode`, `setupCompleted`, `entryClassID`, `selectedObjectives: [String]`, `declaredBonuses: [String: Int]`, `myPotaRefs`, `usedSpots`, `scoreSnapshot`, `schemaVersion: 2`. `myLocation`, `exchangeName` and `exchangeMember` remain as computed views over `sideID`/`sentExchange`; `categoryValues` exposes the nine Cabrillo axes for the operating-time rule; a document from a **newer** schema is refused rather than rewritten. Legacy decode: `myLocation.inState(counties)` → `sideID = "inside"`, `sentExchange.location = counties`; `.outOfState(loc)` → `"outside"` (or the party's single side) and `[loc]`; `exchangeName → name`, `exchangeMember → member`. `nextSerial` is unchanged; `derivedOperatingMode` becomes "the first-listed side runs, every other side searches; a single-side contest runs" — inside runs and outside searches exactly as today.

**`StationProfile`** gains `categoryBand: String?`, `categoryOverlay: String?`, `categoryTime: String?`, and `exchangeDefaults: [String: String]` (my section, zone, check, grid, state, name, power… remembered across contests through `lastStationProfile`). All decode with defaults.
...
- **Phase 0 + phase 1a (foundations) — DONE**, per [`plans/2026-08-17-general-contest-logger-foundations.md`](../plans/2026-08-17-general-contest-logger-foundations.md) (its status block lists every reviewed deviation; the code is authoritative). Merged to master as `1a07949`.
- **Phase 1b (the engine switch) — DONE**, per [`plans/2026-08-17-general-contest-logger-engine-switch.md`](../plans/2026-08-17-general-contest-logger-engine-switch.md), on branch `worktree-general-contest-engine`; full suite **3049 tests, 0 failures**. `ScoreEngine` (692 → 265 lines) now holds only `MultKey`, `ScoreBreakdown`, the shared arithmetic and the `PartyDefinition` bridges; every rule lives once, in `ScoreEngine+Contest.swift`, on `ContestDefinition`. `QSO` carries `sent`/`rcvd` element maps and `ContestLog` carries `sideID`/`sentExchange` (schema 2), with the party-shaped accessors kept as views; both exporters derive their output from the exchange spec; score snapshots, the logs-folder history and the archive exports read `ContestCatalog`. **The app looks and behaves exactly as before**: two oracles say so — the golden equivalence corpus (`Tests/Fixtures/Equivalence/engine-golden.json`, 288 whole `ScoreBreakdown`s over all 50 parties × 3 seeds × both sides, recorded from the pre-switch engine) and the export byte-identity fixtures (`Tests/Fixtures/Exports`, 22 files over eleven party shapes), plus the 70 per-party test files through the bridges.
- **Phase 2 (UI generalisation) — not started.** Plan 3 is unwritten. It carries what phase 1b deliberately left party-shaped: `ExchangeParser` (the entry row still parses through it; `ExchangeValidator` is proven equal on all 50 parties), `MyLocation` (a view over `sideID`/`sentExchange`), `MultClass` (still the six well-known class ids; `MultKey.classID` is the general form), `MultiplierRoster` and `NeededMult` (both still take a `PartyDefinition`), `ExchangeSummary` (still uses `CabrilloExporter.exchangeElement`), and the spec's §2.1–2.5 work — Setup, the entry row, sidebar rosters including the groups reachable through `mapTo: "group"`, the operating-time meter, macros per element, the call-history column map, and the digital seams.
- **Phases 3–4 — not started**: the big five one contest per commit, then the constitution and docs (Article 1 naming cty as authority).

## Resolved in the engine-switch plan (the foundations' carried items)

| Carried item | How it was resolved |
| --- | --- |
| `ScoreEngine.locationContributions` credits one location class per row and breaks a state-code-vs-DXCC-prefix tie by the worked callsign | **Token ownership**: `ExchangeValidator.owningSet(of:in:)` over `ResolvedSets` (built once per element and side) is the one classifier validation and scoring share, so a token feeds at most one enumerated class; **the callsign override** is `Resolver.callsignOverrides` on an entity-counting `dxccEntity` resolver, applied in `ScoreEngine.tokenOwners` before any resolver runs and decided over every class (gated by each resolver's own `sides`), which is exactly today's `dxCountsEntities` gate. `PartyLowering` emits `["states", "provinces"]` for every entity-counting side, never for a sections party. |
| `ExchangeValidator` returns tokens as typed; aliases fold at count time | The `receivedToken` resolver's value is `set.canonical(raw)`; the row keeps what the station sent. |
| A no-home-region party's single side is scored with `outState` | `ContestDefinition.resolvedSideID(_:)` maps a stored side the contest does not declare to its only side (else its last-listed, catch-all side), and logs once when it does. |
| `family` on the four non-state parties; bonus `sides` | Still deferred: `family` is picker grouping (phase 2, where the picker is generalised and those generators are touched); VTQP's W1AW/1 bonus pays both sides today, so a `sides` filter changes its score and belongs in its own VTQP commit. |
| `OperatingTime.compute` takes pre-filtered rows and does not check `appliesTo` | The engine checks `rule.applies(to: log.categoryValues)` and passes the in-mode, in-scope rows (dupes included — a dupe is still time on the air); `Result` gained `offPeriods` for the Cabrillo `OFFTIME:` lines, and `validate()` refuses a rule with `maxMinutes`/`minOffMinutes` below 1. |
| `ContestCatalog.contest(id:)` reloads per call and swallows bundled failures | The bundled load is memoised per bundle and logs each failing file; `PartyCatalog` does the same and now lowers-and-validates on decode, so a party file that cannot lower is refused at load with the model's error. User folders are still read per call. |
| `validate()` does not check `bonuses`/`scoreFactors` references | Added: county-keyed bonuses need a `county` class with a roster, `designatedCountySweep` tokens must be in it, entry-class/objective/declared-bonus ids must be unique, an activated rule's class must list a roster, and `callsignOverrides` must name sets its element accepts. |
| Zone resolvers name their element | The engine reads `resolver.element`. |
| `receivedElements(for:)` omits the call echo | `receivedElements(for:includingCallEcho:)`; the Cabrillo QSO line asks for it and writes the worked call padded to 13. |
| Rosters must add the groups reachable via `mapTo: "group"` | Phase 2 (`MultiplierRoster` is UI-facing and stays party-shaped for now). |
