...
NOTES = (
    ...
    "entities (maximum of 10)'), which dxMultCap records AND WHICH NOW BINDS. The exchange "
    "is the literal 'DX' token, so every entity used to collapse to one multiplier and "
    "the count never reached ten; as of 2026-08-01 the entity is resolved from the WORKED "
    "CALLSIGN against the ARRL DXCC List (January 2026 edition, Resources/DXCC, generated "
    "by gen_dxcc.py), the same split N1MM makes, so twelve entities offered pay the ten "
    "the sponsor allows. KNOWN LIMITATION 1 - "
