...
meqp = {
    ...
    "notes": (
        ...
        "KNOWN SCORING LIMITATION: DXCC entities are multipliers for every entrant with no cap, "
        "but the exchange is 'DX stations send signal report and \"DX\"' — every DX station sends "
        "the same literal token, so this app cannot tell one DXCC entity from another and credits "
        "DX as a SINGLE multiplier per band/mode. Anyone working several DX countries will see a "
        "multiplier count low by one per extra entity per band/mode slot. This is the same "
        "missing DXCC prefix table that limits NHQP, but it bites harder here: there it affected "
        "in-state entrants only and was capped at 10, while MEQP gives DXCC to everyone, uncapped. "
