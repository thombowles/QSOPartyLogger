...
WIQP IS THE SECOND USER OF THE FRACTIONAL-SCORE-MULTIPLIER GAP. Its power
multipliers are QRP x2, LOW x1.5, HIGH x1 - identical to VTQP's, down to the same
three numbers - and ScoreMultipliers is [String: Int]. Nothing ships, for the
same reason as VTQP: {QRP:2, LOW:1, HIGH:1} would understate the commonest power
class by a third while looking correct. Two sponsors now want it, which meets
this repo's own bar for building the field.
...
# The fractional power multipliers, which deliberately do NOT ship. Identical to
# VTQP's. If these change, revisit KNOWN LIMITATION 1 and the deferred gap.
...
wiqp = {
    ...
    "notes": (
        ...
        "https://www.warac.org/wqp/wiqp_rules.htm, with the sponsor's official Multiplier List and "
        "its Cabrillo guide, all read verbatim 2026-07-26. Logs to wiqp-logs@warac.org by 29 "
        "March. SEVEN HOURS in one window, 1800Z to 0100Z on 15 March 2026 - THE SHORTEST TOTAL "
        "OPERATING TIME OF ANY BUNDLED PARTY, beating Illinois's eight; note that Kansas and "
        "Texas each have a SHORTER single session, six hours, but run eighteen hours overall. Round instants with local anchors that "
        "agree under CDT, so nothing had to be reconstructed. (The page carries a stale note, "
        "'First day of Daylight Saving Time!', which is wrong for 2026 - US daylight time began 8 "
        "March, a week earlier - and appears to be leftover copy the sponsor meant to comment out. "
        "It changes nothing.) "
        "READING THE SPONSOR'S CABRILLO GUIDE CHANGED A FIELD, which is worth recording. The rules "
        "say non-Wisconsin stations send 'State or Province or Country', which reads like a DX "
        "prefix; the sponsor's own worked example logs DL6QK as the literal token 'DX'. Had only "
        "the rules page been read, this party would have shipped expecting prefixes and would have "
        "rejected the sponsor's own sample line. The same example also confirms the exchange "
        "carries NO SIGNAL REPORT - 'W9HNW RAC ND9Z BRO' is call and location only - which is the "
        "fifth party after MDC, MNQP, NCQP and IDQP to drop it. Wisconsin stations send a county; "
        "everyone else sends a state, a province, or DX. "
        "Phone 1 point, CW and digital 2. Multipliers count ONCE OVERALL - the sponsor's own "
        "maxima (72 counties, 50 states, 13 provinces) are what settle that, since a per-band or "
        "per-mode scope could not have a maximum of 72. Wisconsin stations count the 72 counties "
        "plus states plus provinces for 135; everyone else counts the 72 counties. WISCONSIN "
        "ITSELF IS A MULTIPLIER, stated outright - 'Wisconsin may be counted as a state "
        "multiplier' - and reachable only through a county, since WI stations send a county. DX "
        "EARNS POINTS BUT NO MULTIPLIER. DC counts as Maryland, which appears NOWHERE in the rules "
        "and only in the multiplier list, printed as the single row 'MD Maryland/(D.C.)'. Ten "
        "bands - the largest list of any bundled party - derived from the suggested-frequency "
        "table, since the rules publish no list and say only 'all amateur bands not prohibited for "
        "contesting'; that table covers 160 through 10 m plus 6 m, 2 m, 1.25 m and 70 cm. "
        "COUNTY-LINE OPERATION IS FORBIDDEN: 'Mobiles or portables may not sit on a county line', "
        "so a two-county entry is rejected, as in ALQP and MNQP. FT8 and FT4 are not accepted "
        "while RTTY, PSK, Olivia and Feld-Hell are, which is below this app's mode-class "
        "granularity - KEEP FT8/FT4 OUT OF THE LOG. All digital modes count as one, stated "
        "outright ('Cannot work the same station on more than one Digital mode on the same band'), "
        "which is exactly how this app behaves. "
        "KNOWN LIMITATION 1 - THE POWER MULTIPLIER IS NOT APPLIED, AND IT IS THE BIGGEST NUMBER "
        "HERE. The sponsor's scoring is points x POWER x multipliers + bonuses, where QRP (under "
        "5 W) is x2, LOW POWER (5-100 W) is x1.5 and high power is x1. This app's score multiplier "
        "field holds whole numbers only, so x1.5 cannot be represented, and shipping x1 for low "
        "power would understate a low-power entrant by a third while looking right. Nothing ships "
        "instead: MULTIPLY THE FINAL SCORE YOURSELF - by 2 for QRP, by 1.5 for low power - before "
        "submitting. THIS IS THE SAME GAP VERMONT HAS, with the identical three factors, so two "
        "sponsors now want it. KNOWN LIMITATION 2 - the 500-point county bonus is credited for "
        "EVERY county with 12 or more QSOs, including your home county, while the sponsor pays it "
        "only 'for each county that you operate from OUTSIDE your home county'. A Wisconsin mobile "
        "or portable who also makes 12+ QSOs from home is over-credited 500; subtract it. "
        "IN-STATE MOBILES AND PORTABLES ONLY. KNOWN LIMITATION 3 - the W9FK bonus is credited on "
        "every band and mode, while the sponsor pays it only BELOW 50 MHz. Working W9FK on 6 m, "
        "2 m, 1.25 m or 70 cm over-credits 100 points per slot; subtract those. "
        "Cabrillo CONTEST value WI-QSO-PARTY, printed by the sponsor in its own Cabrillo guide, so "
        "this does not rest on the WA7BNM registry. Counties are 72 with uniform 3-letter codes "
        "and several dense clusters: THREE Mar- counties of which only Marathon is MAR (Marinette "
        "is MRN, Marquette MRQ); THREE Gr- counties (Grant GRA, Green GRE, Green Lake GRL - the "
        "last two one letter apart); and SIX W- counties (Walworth WAL, Washington WAS, Washburn "
        "WSB, Waukesha WAU, Waupaca WAP, Waushara WSR), where neither Washburn nor Waushara takes "
        "the naive form. Multi-word names are printed without punctuation: Fond du Lac FON, La "
        "Crosse LAC, St Croix STC (no period, as in Minnesota's St Louis). "
        "OPEN QUESTIONS (why this is partial): (1) KNOWN LIMITATION 1 - the fractional power "
        "multiplier, which changes the final score for every entrant who is not running high "
        "power. (2) and (3) - the two bonus rules above, each of which over-credits a narrow case. "
        "None of the three is a rule in doubt; all three are missing app features. Note also that "
        "the sponsor's do-not-use list names 906.50 and 1294.50 MHz, so 33 cm and 23 cm contacts "
        "are plausibly legal here and cannot be logged at all - the same gap NYQP has."
    ),
}
...
print(f"  NOT shipped: the x2 / x1.5 / x1 power multiplier - SECOND USER after VTQP")
