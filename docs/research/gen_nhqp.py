...
nhqp = {
    ...
    "multipliers": {
        ...
        "inState": {
            ...
            "countScope": "once",
            # "up to 10 DXCC country" — a real cap now that entities can be
            # told apart. The exchange carries only the literal token "DX", so
            # the entity comes from the worked callsign (ARRL DXCC List, via
            # Resources/DXCC).
            "dxMultCap": 10,
    ...
    "notes": (
        ...
        "read verbatim 2026-07-24. THE 10-DXCC ALLOWANCE IS REAL AGAIN as of 2026-08-01. "
        "The rules give NH stations 'up to 10 DXCC country' as multipliers while the "
        "exchange is 'DX: RS(T) + \"DX\"', so every DX station sends the same literal token "
        "and this app used to credit DX as a SINGLE multiplier — an NH entrant's count ran "
        "up to 9 low, and dxMultCap recorded a rule it could never enforce. The entity now "
        "comes from the WORKED CALLSIGN instead of the exchange field, resolved against the "
        "ARRL DXCC List (January 2026 edition, Resources/DXCC, generated by gen_dxcc.py) — "
        "the same split N1MM makes, and the only evidence available when the exchange "
        "carries no country. Ten distinct entities now count ten, and dxMultCap binds. "
        "RESIDUAL: a callsign whose prefix the ARRL list shares between entities resolves "
        "to the designated one, and a station signing only a portable suffix may not "
        "resolve at all, in which case the contact falls back to a single DX multiplier "
        "rather than being lost. Out-of-state entrants are unaffected either way: DX is "
        "not one of their multiplier "
