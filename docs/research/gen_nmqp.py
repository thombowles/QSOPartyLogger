...
nmqp = {
    ...
    "multipliers": {
        ...
        "inState": {
            ...
            # "Each multiplier may be counted ONLY ONCE, regardless of mode or band."
            "countScope": "once",
            # "DX entities worked" - individually, which the packet's own
            # sample log (LY2ZZ 599 DX) makes impossible from the exchange
            # field alone, so the entity comes from the callsign.
    ...
    "notes": (
        ...
        "THIS PARTY'S POWER MULTIPLIER FITS, which is worth saying because two others' do not. "
        "QRP is x5, low power x2 and high power x1 - all whole numbers - so unlike Vermont and "
        "Wisconsin, whose low-power factor is x1.5 and cannot be represented, NEW MEXICO'S POWER "
        "MULTIPLIER IS APPLIED IN FULL. QRP at x5 is the largest score multiplier in this app. "
        ...
        "DX ENTITIES COUNT ONE BY ONE as of 2026-08-01. The multiplier rule counts 'DX entities "
        "worked' individually while the log format carries the literal token DX - the packet says "
        "the QTH field is a county, a state or province, or 'DX', and its own sample log shows "
        "LY2ZZ 599 DX - so a New Mexico entrant working ten DXCC entities used to be credited ONE "
        "multiplier instead of ten. THE SPONSOR'S OWN AUTOMATED SCORER FACES THE SAME PROBLEM, "
        "since the field it reads is DX too, and must derive the entity from the callsign. That "
        "is now exactly what this app does: the entity is resolved from the WORKED CALLSIGN "
        "against the ARRL DXCC List (January 2026 edition, Resources/DXCC, generated by "
        "gen_dxcc.py), which is also how N1MM does it. That sample log's LY2ZZ resolves to "
        "Lithuania. OUT-OF-STATE ENTRANTS ARE UNAFFECTED either way - their only multipliers are "
        "NM counties. "
        ...
        "OPEN QUESTION (why this is partial): the W1AW/5 bonus is explicitly 2026-only and must "
        "be removed for 2027; the generator asserts the marker is still present so its "
