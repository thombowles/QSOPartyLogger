...
- **North American QSO Party, CW** (Jan 10 + Aug 1, 2026) — the first bundled
  ...
  **DC as its own multiplier**, the standard 13 provinces, and **46 of its 47
  ...
  [`gen_naqp.py`](docs/research/gen_naqp.py) with names joined from the ARRL
...
- **QSO Party Hub spots**: several sponsors point their operators at
  ...
  multipliers. 39 of the 46 bundled parties are covered — the hub serves
  state and provincial parties, so the exceptions are California (an
...
Not every bundled party is covered, and the app says so rather than polling a
dead page: California's hub page is an unfinished sponsor template, WA Salmon
Run has no page at all, and the hub does not serve the multi-state entries or
NAQP.
...
(`brew install xcodegen`). 1866 unit tests cover the scoring engine, county
...
- NAQP CW + SSB: rules from NCJ's own "Rules: 2026 North American QSO Party
  ...
  [`docs/research/gen_naqp.py`](docs/research/gen_naqp.py) from the committed
...
- **DXCC entities, all parties.** `Resources/DXCC/dxcc_entities.json` is the
  **ARRL DXCC List, Current Entities, January 2026 Edition**
  ...
  generated by [`docs/research/gen_dxcc.py`](docs/research/gen_dxcc.py) from the
