...
/// prove `isConnected` and wire a real CW sender, but a real K3 never gets to
/// speak, so `voiceStatus` never becomes ready on its own.
...
final class RadioControllerVoiceTests: XCTestCase {
    ...
    /// A K3 on a silent port. A K3 has key lines, so CW here goes out the
    /// direct keyer against `/dev/null` — there is no backend left to choose
    /// (Article 11). These tests never wait on the keying thread: `sendCW`
    /// sets `nowSending` synchronously, which is the only part they assert on.
